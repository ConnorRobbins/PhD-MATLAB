...
wave_k=1;
%wave_k=sqrt(-6*(Froude-1)); %sqrt(-3mu) 
%wave_k=sqrt((9*amplitude/4)-6*(Froude-1)); %right intersection point in kdv
%wave_k=sqrt(-((9*amplitude/4)+6*(Froude-1))); %left intersection point in kdv
...
rankSVD=561; %WILL BE OVERWRITTEN IF auto_pick_unperturbed_rank=1;   The fixed truncation rank for applying TSVD to the unperturbed problem BE OVERWRITTEN IF auto_pick_unperturbed_rank=1;   The fixed truncation rank for applying TSVD to the unperturbed problem
...
n_turningpoint_cutoff_nonlinear=4;
...
%%%% Also transform Thetas to have a look
[theta_b_frequencies_nonlinear_middle_only,theta_b_yshift_nonlinear_middle_only] = FUN_FourierTransform_topography(Theta_b_chosen(left_index_nonlinear:right_index_nonlinear),middle_L,middle_N);
[theta_b_pos_frequencies_nonlinear_middle_only,theta_b_pos_yshift_nonlinear_middle_only] = FUN_FourierTransform_post_clean(theta_b_frequencies_nonlinear_middle_only,theta_b_yshift_nonlinear_middle_only);

%%%% Also transform Theta to have a look
[theta_frequencies_nonlinear_middle_only,theta_yshift_nonlinear_middle_only] = FUN_FourierTransform_topography(Theta(left_index_nonlinear:right_index_nonlinear),middle_L,middle_N);
[theta_pos_frequencies_nonlinear_middle_only,theta_pos_yshift_nonlinear_middle_only] = FUN_FourierTransform_post_clean(theta_frequencies_nonlinear_middle_only,theta_yshift_nonlinear_middle_only);
...
%% Report max amp frequencies

{strcat('nonlinear_freq_max=',num2str(max_amp_freq_nonlinear_middle_only));strcat('kdv_freq_max=',num2str(max_amp_freq_kdv_middle_only))}
...
title('theta b middle only')
stem(theta_b_pos_frequencies_nonlinear_middle_only,theta_b_pos_yshift_nonlinear_middle_only,'-r','MarkerSize',4)
stem(theta_pos_frequencies_nonlinear_middle_only,theta_pos_yshift_nonlinear_middle_only,'--b','MarkerSize',4)
xlim([0,5*wave_k])
ylim([0,0.005])