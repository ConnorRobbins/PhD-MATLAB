...
A=0.2;
mus=[0.55,0,-0.55];
% mu is 2(F-1) here not (F-1)

figure(17); clf; hold on;

for i=1:numel(mus)
    mu=mus(i);
    plot(k,abs(A*(mu+(k.^2)/3)),LineWidth=2)
end

yline(A*0.8+0.1,'--k',LineWidth=1.5)
yline(A*0.55-0.02,':k',LineWidth=2)
ylim([0,0.4])
...
arrowposx=[0.56,0.46];
arrowposy=[0.15,0.11];
annotation('textarrow',arrowposx,arrowposy,'String','$\nu=\sqrt{-6\mu}$','Interpreter','latex','FontSize',12);
...
annotation('textarrow',arrowposx,arrowposy,'String','$c_1(0)=2|A\mu|$','Interpreter','latex','FontSize',12);
