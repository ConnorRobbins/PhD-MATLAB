...
N_t=651;         %Number of points on topography
...
Random_error_Number=1; %Number of times to perturb free surface and solve
...
random_noise_std=1E-2; % standard deviation of the normal dist. from which noise is drawn

auto_pick_unperturbed_rank=1; %If this is set to 1 the code will ignore rankSVD set below and instead decide on rankSVD based on going 80% along the flat part of the norm curve 
...
%%% Perturb the surface once to simulate measurement with half the std 
[closestval,neg_range_index]=min(abs(Phi_s+Noise_range)); %Identify the indices of all surface points to which noise will be added
...
Ys_measurement=Ys;
Ys_measurement(neg_range_index:pos_range_index)=Ys(neg_range_index:pos_range_index)+0.5*random_noise_std*randn(1,noise_range_entries);


[Yt_std,Ys_std,Ys_noisy_store,Yt_noisy_store,Theta_b_store,truncated_inv,Yt_noisy_average]=func_perturbed_surface_problem(L,Froude,N_s,N_t,P,Ys_measurement,Phi_s,Phi_t,rankSVD_noisy,U,singularvals,V,unperturbed_endpoints,Noise_range,Random_error_Number,random_noise_std);
...
plot(Phi_s,Ys_measurement,'-k')
ylabel('$Y_ f$','Interpreter','latex','Rotation',0,'FontSize',16)
legend('True surface','Example of perturbed surface','Measured surface')
