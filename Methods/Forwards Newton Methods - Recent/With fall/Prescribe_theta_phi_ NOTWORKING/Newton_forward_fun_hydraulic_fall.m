function [Phi,Xs,Ys,Xt,Yt,Theta,F] = Newton_forward_fun_hydraulic_fall(L,N,F_init_guess,amplitude,b_width,ijac,P,Theta_init_guess,Theta_b)


    %Set up initial guess for theta
    Theta=reshape(Theta_init_guess,N,1);
    Theta_bottom=reshape(Theta_b,N,1);
    F=F_init_guess;
...
%             save('JAC1.mat','jac1')
    ...
    for i=1:N-1
        ...
        Tau_mid_t(i) = trapz(Phi,(transpose(Theta).*Tau_integrand_surface)) + trapz(Phi,(Theta_b.*Tau_integrand_bottom))  ;
    end
    ...
    %Solve for yt from dy/dphi
    coeffmat=diag(ones(N,1),0)-diag(ones(N-1,1),1); coeffmat(1,2)=0;
    rhsmat=zeros(N,1);
    rhsmat(2:N-1)= - (exp (-Tau_mid_t(2:end)) .* sin(Theta_mid_t(2:end)) * del_Phi);
    
    Yt=linsolve(coeffmat,rhsmat);
