function [Phi,Xs,Ys,Xt,Yt,Theta,Theta_bottom,F,c] = Newton_forward_fun(L,N,F,amplitude,b_width,ijac,P,Theta_init_guess,Theta_b_init_guess,c_init_guess)
    ...
    w=[Theta(2:N-1);Theta_bottom(1:N-1);c_init_guess];
...
%             save('JAC1.mat','jac1')
    ...
    Theta = [0; w(1:N-2); 0];  % The 0 is the value of theta at last grid point (BC for subcritical)
    Theta_bottom=[w(N-1:end-1);0];
    c=w(end);
    ...
    for i = 1 : N - 1
        ...
        G(N:end)=Yt(1:end-1)- (amplitude*exp(-(b_width*(Phi(1:end-1)-c)).^2))' - (amplitude*exp(-(b_width*(Phi(1:end-1)+c)).^2))';
