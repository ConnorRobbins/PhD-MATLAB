function [Phi,Xs,Ys,Xt,Yt,Theta,Theta_bottom,P,jac,jac_sol] = eqsolve_forward(L,N,F,iload,isave,ijac,Pressure,Topography,amplitude,b_width)
    ...
    if Topography ==1
        Theta_bottom=atan(-2*amplitude*b_width*Phi.*exp(-(b_width*Phi).^2));
        %Theta_bottom=-0.4*Phi.*exp(-(Phi).^2);
    else
        Theta_bottom=zeros(1,N);
    end
    
    if Pressure ==1
        P = -0.1*exp(-(Phi).^2); % initial pressure distribution
    else 
        P = zeros(N,1);
    end
    ...
    if iload==2
        s= load ('Forunforced_thetaguess.mat');
        Theta=s.Theta_surface
    end
